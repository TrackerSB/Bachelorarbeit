...
% ****************************************************************************************************
\newcommand{\myTitle}{Boundaries of Polyhedral Compilation\xspace}
\newcommand{\mySubtitle}{An Empirical Study\xspace}
...
\newcommand{\myFaculty}{Faculty for Computer Science and Mathematics\xspace}
\newcommand{\myDepartment}{Department\xspace}
\newcommand{\myUni}{University Passau\xspace}
...
\newcommand{\legend}[0]{
    \begin{table}[H]
        ...
        \begin{tabularx}{\textwidth}{CX}
            \begin{tikzpicture}
                \coordinate(left);
                \coordinate[right=of left] (right);
                \path[llvmIrPath] (left) to (right);
            \end{tikzpicture} & Passing LLVM-IR\\
            \begin{tikzpicture}
                \node[llvmIrNode] {Node};
            \end{tikzpicture} & Node using LLVM-IR\\
            \begin{tikzpicture}
                \coordinate(left);
                \coordinate[right=of left] (right);
                \path[nonLlvmIrPath] (left) to (right);
            \end{tikzpicture} & Passing non-LLVM-IR\\
            \begin{tikzpicture}
                \node[nonLlvmIrNode] {Node};
            \end{tikzpicture} & Node not using LLVM-IR\\
        \end{tabularx}
    \end{table}
}

\newenvironment{tikzlegend}[0]{
    \begin{tikzpicture}
}{
    \end{tikzpicture}\\
    %\legend %FIXME Legend increasing table number
}

%Has to be loaded after graphicx
\usepackage[svgpath=svg/,clean]{svg}
...
\newcommand{\piechart}[3]{
    \begin{figure}
        ...
        \def\angle{0}
        \def\radius{3}
        \def\cyclelist{{"orange", "blue", "red", "green", "yellow", "cyan", "magenta"}}
...
\newcommand{\undefv}{\texttt{undef}\xspace}
\newcommand{\generator}{LLVM Code Generator\xspace}
\newcommand{\gpoly}{\ac{GPoly}\xspace}
\newcommand{\ir}{\ac{IR}\xspace}
\newcommand{\linker}{LLVM Linker\xspace}
\newcommand{\llvmir}{\ac{LLVM-IR}\xspace}
\newcommand{\llvm}{\ac{LLVM}\xspace}
\newcommand{\lnt}{\ac{LNT}\xspace}
\newcommand{\opt}{LLVM Optimizer\xspace}
\newcommand{\papi}{\ac{PAPI}\xspace}
\newcommand{\pcp}{\ac{PCP}\xspace}
\newcommand{\scop}{\ac{SCoP}\xspace}
\newcommand{\scops}{\ac{SCoP}s\xspace}
