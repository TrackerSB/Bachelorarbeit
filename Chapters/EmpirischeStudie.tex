...
\begin{itemize}
    ...
    \item Unsupported terminator instruction\\
        This is occurring when a flow breaking instruction like \mintinline{c++}{return} or \mintinline{c++}{goto} is called within a loop.
    \item Unreachable in exit block (\autoref{lst:unreachableExitBlock})\\
        This is appearing when \mintinline{c++}{unreachable()} is called.\cite{llvmUnreachable}
    ...
    \item Undefinded base pointer\\
        \drafnote{It is raised when a pointer holds the value \undefv.}
    ...
    \item Stack allocations\\
        Stack allocations like using \mintinline{c++}{alloca} can not be handled.
...
Even if there are a lot of \scops which are processed the big \scops and the -- according to the execution time -- big \socps may not be able to be optimized.\\
...
The first experiment is comparing the ratio of \scops \(DynCov_s\) and their parents \(DynCov_p\).\\
