...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{Xccccc} \toprule
        ...
        Test inputs                              & \(TI\)  & Con.   & ?     & - & -\\
        \midrule
        Execution time of \scops                 & \(T_s\) & Dep.   & Ratio & - & \(\mathbb{Q}\)\\
        Execution time of parents                & \(T_p\) & Dep.   & Ratio & - & \(\mathbb{Q}\)\\
        Ratio of \scops                          & \(R_s\) & Dep.   & Ratio & - & \(\mathbb{Q}\)\\
        Ratio of parents                         & \(R_p\) & Dep.   & Ratio & - & \(\mathbb{Q}\)\\
...
\(H_1\): As result the \scops should have quite the maximum size, which is limited due to the fact that toplevel regions can not be classified as \scops by Polly per default.
...
\(H_2\): The theoretical speedup \(S_s\) is also going to be high but significally lower than \(S_p\) which represents the speedup when every parent of any \scop could also be optimized by Polly.
